...
/*EDITED by Shaun Hotchkiss and Seshadri Nadathur
    Edits account for edge/boundary particles and for particle
    densities modified due to variable selection functions
    (i.e., density != 1/volume in all cases)
*/
...
#define NLINKS 300000 /* Number of possible links with the same rho_sl */
#define FF fflush(stdout)
#define FNL 256 /* Max length of filenames */

typedef struct Particle {
  realT dens;
  realT trvol; /* ADDED BY SHAUN */
  int nadj;
  int nadj_count;
  int *adj;
...
typedef struct Zone {
  ...
  realT leak; /* Density of last leak zone*/
  int *adj; /* Each adjacent zone, with ... */
  realT *slv; /* lowest linking density (originally Smallest Linking Volume) */
  ...
  int  edge; /* Does the zone neighbour any border particles ADDED BY SHAUN */
} ZONE;
...
int main(int argc,char **argv) {
  ...
  FILE *adj, *vol, *zon, *vod, *txt, *trvol; /* EDITED BY SHAUN */
  PARTICLE *p;
  ZONE *z;
  ZONET *zt;
  char *prefix, adjfile[FNL], volfile[FNL], trvolfile[FNL], zonefile[FNL], voidfile[FNL], txtfile[FNL]; /* EDITED BY SHAUN */
  int i, j,k,l, h, h2,hl,n,np, np2, np3, nzones, nrealzones, nhl, nhlcount, nhl2; /* EDITED BY SHAUN */
  int nvoidsreal;
  ...
  if (argc != 5) {
    ...
    printf("arg4: Flagged border-particle density (0 for default 0.9e30 (voids), 1.1e-30 (clusters))\n\n");
    exit(0);
  }
  ...
  sprintf(trvolfile,"%s.trvol",prefix); /* ADDED BY SHAUN */
  ...
  if (obo == 'c'){
    ...
    if (voltol <= 0.) {
      ...
      sprintf(zonefile,"%sc.%lf.zone",prefix,voltol);
      sprintf(voidfile,"%sc.%lf.void",prefix,voltol);
      sprintf(txtfile,"%sc.%lf.txt",prefix,voltol);
    }
  }
  if (borderdens == 0.) {
  ...
  /* Read the (modified) Voronoi volumes from .vol file to get VTFE densities*/
  vol = fopen(volfile, "r");
  if (vol == NULL) {
  ...
  for (i=0;i<np;i++) {
    ...
    /*although it is called p[i].dens, we are actually initially reading modified Voronoi VOLUMES*/
    if ((p[i].dens < 1e-30) || (p[i].dens > 1e30)) {
      printf("Whacked-out volume found, of particle %d: %g\n",i,p[i].dens);
      p[i].dens = 1.;
    }
    if (obo == 'v'){
      p[i].dens = 1./p[i].dens; /* for void-finding, p[i].dens now contains densities */
    }
    /*for cluster-finding, p[i].dens still contains volumes, so the algorithm now proceeds the same */
  }
  fclose(vol);
  ...
  /* Open true volume file to read true volumes of particles and then zones ADDED BY SHAUN */
  trvol = fopen(trvolfile, "r"); /* ADDED BY SHAUN */
  fread(&np3,1, sizeof(int),trvol); /* ADDED BY SHAUN */
  if (np3 != np2) { /* ADDED BY SHAUN */
    printf("Number of particles doesn't match! %d != %d\n",np3,np2); /* ADDED BY SHAUN */
    exit(0); /* ADDED BY SHAUN */
  } /* ADDED BY SHAUN */
  for (i=0;i<np;i++) { /* ADDED BY SHAUN */
    fread(&p[i].trvol,1,sizeof(realT),trvol); /* ADDED BY SHAUN */
  } /* ADDED BY SHAUN */
  
  for (i=0; i<np; i++) {
    h = zonenum[jumped[i]];
    if (p[i].dens < borderdens) { /*  EDITED BY SHAUN */
      z[h].vol += p[i].trvol; /* EDITED BY SHAUN */
    }
    else {
  ...
  maxdenscontrast = 0.;
  for(i=0;i<np; i++){
    if ((p[i].dens > maxdens)&&(p[i].dens < borderdens)) maxdens = p[i].dens; /* EDITED BY SHAUN */
    if (p[i].dens > maxdens_border) maxdens_border = p[i].dens;
    if ((p[i].dens < mindens)&&(p[i].dens > invborderdens)) mindens = p[i].dens; /* EDITED BY SHAUN (last edit regarding borderdens. one more edit regarding edge zones) */
  ...
  /* sorter no longer sorts, now it just serves as a flag for real structures */
  nvoidsreal = 0;
  for (h=0; h< nzones; h++) {
    if (p[z[h].core].dens < borderdens) {  /* EDITED BY SHAUN */
  ...
  for (i=0; i<nzones; i++) {
    ...
    if (sorter[i] > 0.) {
      fprintf(txt,"%d %d %d %e %e %d %d %e %d %lf %6.2e\n",
	      i, z[i].edge, z[i].core, p[z[i].core].dens, z[i].vol, z[i].np, z[i].nhl, z[i].voljoin, z[i].npjoin, z[i].denscontrast, prob); /* EDITED BY SHAUN to include whether the central zone is on the border or not */
    }
